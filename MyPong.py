...
def new_game():
    ...
    score1 = 0
    score2 = 0
    #random for LEFT or RIGHT at the beginning of a new game
    spawn_ball(random.choice([LEFT, RIGHT])) 
    paddle1_pos = paddle2_pos = HEIGHT / 2 # put paddles in centre
...
def draw(canvas):
    ...
    # draw scores
    canvas.draw_text(str(score1), [WIDTH / 4,110], 50, "White", 'sans-serif')
    canvas.draw_text(str(score2), [WIDTH - WIDTH / 4,110], 50, "White", 'sans-serif')
        
        
def keydown(key):
    global paddle1_vel, paddle2_vel
    # also support "e" key because "w" is broken in my keyboard
    if key == simplegui.KEY_MAP["down"]:
        ...
        paddle1_vel = 6
    elif key == simplegui.KEY_MAP["e"] or \
