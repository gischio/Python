__author__ = "Giovanni Schiochet"
__copyright__ = "None"
__credits__ = ["None"]
__license__ = "GPL"
__version__ = "1.0.1"
__maintainer__ = "Gischio"
__email__ = "schiochet70@gmail.com"
__status__ = "Coursera Training course"
...
class Ship:
    ...
    def update(self):
        ...
        if self.thrust == True:
            ...
            self.vel[0] += 0.9 * forward[0]
            self.vel[1] += 0.9 * forward[1]
            #ship_thrust_sound.play()
        else:
            ship_thrust_sound.pause()
            
    def thrustToggle(self, thrust):
        self.thrust = thrust
...
def draw(canvas):
    ...
    canvas.draw_image(debris_image, center, size, (wtime + WIDTH / 2, HEIGHT / 2), (WIDTH, HEIGHT))
    canvas.draw_text("Lives: " + str(lives), [670,35], 36, "White", "sans-serif")
    canvas.draw_text("Score: " + str(score), [5,35], 36, "White", "sans-serif")
...
def rock_spawner():
    ...
    a_rock = Sprite([random.randrange(WIDTH), random.randrange(HEIGHT)], \
                    [random.choice([1, -1]) * random.random() * 2, \
                     random.choice([1, -1]) * random.random() * 2], \
                    1, random.choice([0.5, -1]) * 0.05, asteroid_image, asteroid_info)
